...
beforeAll(() => {
  ...
  fs.writeFileSync(
    ...
    `
import fs from 'node:fs';
const [moduleUrl, stateDir, thread, role, entered, release, lingerMs] = process.argv.slice(2);
const m = await import(moduleUrl);
const env = { SPARROW_STATE_DIR: stateDir };
const say = (o) => process.stdout.write(JSON.stringify({ pid: process.pid, ...o }) + '\\n');

if (role === 'holder') {
  // At the TOP of the critical section: the lock is held and nothing is renamed
  // yet, so the contender passes its pre-check and must wait on the lock — the
  // only state in which contention (rather than a pre-check refusal) is visible.
  m.__setAwaitLockedHookForTests(() => {
    fs.writeFileSync(entered, String(process.pid));
    const until = Date.now() + 15000;
    while (!fs.existsSync(release) && Date.now() < until) { /* spin */ }
  });
}
...
const gen = m.prepareAwaitGeneration({ env, kind: 'await:codex', thread });
try {
...
describe('two processes publishing at once', () => {
  it('serialises them: the contender waits, then REFUSES a live different thread', async () => {
    ...
    expect(JSON.parse(fs.readFileSync(path.join(stateDir, 'await-arming.lock'), 'utf8')).pid).toBe(a.pid);
    ...
    expect(fs.existsSync(path.join(stateDir, 'await-arming.lock'))).toBe(false);
  }, 60_000);
  ...
  it('same thread: the contender supersedes, newest-wins, no refusal', async () => {
    ...
    await waitForFile(entered);
    const b = arm({ stateDir, thread: 'thread-same' });
    await nap(400);
