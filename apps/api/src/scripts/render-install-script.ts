 ...
 *
 * `--cache-bust <token>` stamps `?v=<token>` onto the two bundle URLs (the
 * website build passes the bundle build stamp) so an edge cache can never hand
 * a fresh install the previous release's bundle.
 *
...
export function installScriptFor(base: string = DEFAULT_INSTALL_URL, cacheBust?: string): string {
  return renderInstallScript(base, cacheBust);
}

const USAGE = `usage: render-install-script --base ${DEFAULT_INSTALL_URL} --out <file> [--cache-bust <token>]`;
...
async function main(argv: readonly string[]): Promise<void> {
  ...
  const body = installScriptFor(optionalString(args, 'base'), optionalString(args, 'cache-bust'));
