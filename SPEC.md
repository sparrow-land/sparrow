...
| `LOG_LEVEL` | `info` at the container entrypoint (`false`/off for an embedded `buildServer()`) | pino level: `fatal`\|`error`\|`warn`\|`info`\|`debug`\|`trace`. `off`\|`false`\|`none`\|`silent`\|`0` disable the logger. An EMPTY value — which compose's `${LOG_LEVEL:-}` always defines — reads as unset and resolves to `info`; only an embedded `buildServer()` given no level at all starts silent. An unrecognized level degrades to `info` rather than throwing at boot. The startup banner is a log record like any other, so `off` really is silent (a failed boot still writes to stderr); the startup banner printed to stdout at listen time obeys it too. Log records are JSON; when stdout is a TTY and `NO_COLOR` is unset (`FORCE_COLOR` opts a pipe in) they are rendered by `pino-pretty` as coloured single lines instead — the same rule the banner colours by. `docker logs`, a redirected file and the scenario harness therefore always receive JSON. `req.headers.authorization`, `req.headers.cookie` and the response `set-cookie` are redacted |
| `SPARROW_NO_BANNER` | *(unset)* | set (any value but empty/`0`/`false`) to skip the startup banner — the Sparrow art (or the illustration itself on an allowlisted graphics terminal, see `SPARROW_BANNER_IMAGE`), version + build, the URL to open, the docs home — that the server prints to stdout once `listen` resolves. Colour only when stdout is a TTY and `NO_COLOR` is unset (`FORCE_COLOR` opts in). `LOG_LEVEL=off` also silences it |
| `SPARROW_BANNER_IMAGE` | *(unset = allowlist decides)* | forces the startup banner's illustration on (`1`\|`true`\|`on`) or off (`0`\|`false`\|`off`); any other value, empty included, reads as unset. Unset, the server draws the real sparrow PNG (kitty graphics protocol, `f=100` direct transmission, chunked at 4096 base64 bytes, each chunk quiet (`q=2`) so a terminal that fails mid-transfer prints no error, placed in a 20×10 cell box with `C=1` and the text block printed below it) only when stdout is a TTY **and** the terminal is on a fixed ALLOWLIST known to render it without prompting: kitty (`TERM=xterm-kitty`, `KITTY_WINDOW_ID`), Ghostty (`TERM_PROGRAM=ghostty`, `TERM=xterm-ghostty`, `GHOSTTY_RESOURCES_DIR`), WezTerm (`TERM_PROGRAM=WezTerm`). Never inside tmux/screen (`TMUX` set, or `TERM` starting `screen`/`tmux`: passthrough is unreliable) and never in iTerm2 (`TERM_PROGRAM=iTerm.app`, `LC_TERMINAL=iTerm2`), which can render but may prompt. Everywhere else, and on any non-TTY, the ASCII bird. `1` is the escape hatch for `docker run -it`, where the host terminal's env is invisible inside the container — it still requires a TTY. When the env says nothing either way — no allowlist hit, not tmux/screen, not iTerm2 — the server **asks the terminal at runtime** instead of falling straight back to ASCII: on a TTY it writes a kitty graphics feature query (`ESC _ G i=31,s=1,v=1,a=q,t=d,f=24;AAAA ESC \`, which draws and stores nothing and carries no `q=2`, so the reply is not suppressed), then XTVERSION (`ESC [ > 0 q`), then a DA1 request (`ESC [ c`), and draws the illustration only when BOTH `ESC _ G i=31;OK ESC \` **and** an XTVERSION reply (`ESC P > | <name> ESC \`) naming one of the same three allowlisted terminals — kitty, Ghostty, WezTerm, matched case-insensitively on the leading name — arrive before the DA1 reply (`ESC [ ? … c`), which is the fence. `OK` alone is not enough: iTerm2 3.5 answers the query `OK` and then fails the transmission, and inside a container its `TERM_PROGRAM` is invisible, so the name is the only thing that can exclude it. No reply within 500 ms, an error reply, an unknown or missing name, or a bare DA1 all mean the ASCII bird; the probe always waits for the DA1 fence (or the timeout) before restoring the terminal, so no reply it asked for can be echoed onto the screen afterwards — this is what makes `docker run -it` from kitty/Ghostty draw the picture even though the host terminal's env is invisible inside the container. `SPARROW_NO_BANNER` and `LOG_LEVEL=off` suppress both renderings |
| `SPARROW_BANNER_PROBE` | *(unset = on)* | set to `0`\|`false`\|`off` to never write a feature query to the terminal; any other value, empty included, reads as unset. It disables the runtime probe **alone** — `SPARROW_BANNER_IMAGE` and the terminal allowlist still decide the banner exactly as above, so an allowlisted terminal still gets the illustration and everything else gets the ASCII bird. The probe is already skipped, without this variable, whenever stdout is not a TTY, inside tmux/screen, in iTerm2, and when `SPARROW_BANNER_IMAGE` has forced the answer either way |
