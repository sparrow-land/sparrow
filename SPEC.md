...
| `LOG_LEVEL` | `info` at the container entrypoint (`false`/off for an embedded `buildServer()`) | pino level: `fatal`\|`error`\|`warn`\|`info`\|`debug`\|`trace`. `off`\|`false`\|`none`\|`silent`\|`0` disable the logger. An EMPTY value — which compose's `${LOG_LEVEL:-}` always defines — reads as unset and resolves to `info`; only an embedded `buildServer()` given no level at all starts silent. An unrecognized level degrades to `info` rather than throwing at boot. The startup banner is a log record like any other, so `off` really is silent (a failed boot still writes to stderr); the ASCII banner printed to stdout at listen time obeys it too. Log records are JSON; when stdout is a TTY and `NO_COLOR` is unset (`FORCE_COLOR` opts a pipe in) they are rendered by `pino-pretty` as coloured single lines instead — the same rule the banner colours by. `docker logs`, a redirected file and the scenario harness therefore always receive JSON. `req.headers.authorization`, `req.headers.cookie` and the response `set-cookie` are redacted |
| `SPARROW_NO_BANNER` | *(unset)* | set (any value but empty/`0`/`false`) to skip the ASCII startup banner (Sparrow art, version + build, the URL to open, the docs home) that the server prints to stdout once `listen` resolves. Colour only when stdout is a TTY and `NO_COLOR` is unset (`FORCE_COLOR` opts in). `LOG_LEVEL=off` also silences it |
